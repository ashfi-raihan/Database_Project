...
public class SignUpPage implements ActionListener{ 
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource() == deleteBtn){
			...
			if(!upass.isEmpty()){
			UserFileIO.removeUser(uname,upass);
		...
		if(e.getSource() == addBtn){
			...
			UserFileIO.addUser(uname,upass,email.getText(),id.getText());
			JOptionPane.showMessageDialog(frame, "User Added in the house","Information", JOptionPane.INFORMATION_MESSAGE);
			LoginPage lg = new LoginPage();
			}
			else{
				JOptionPane.showMessageDialog(frame, "Put at least Username and Password","ERROR", JOptionPane.ERROR_MESSAGE);
			}
		}
