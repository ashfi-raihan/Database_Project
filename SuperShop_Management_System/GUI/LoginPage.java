...
import File.UserIO.*;
import GUI.*;
import GUI.Employee.EmployeePage;
import GUI.Product.ProductPage;
...
public class LoginPage implements ActionListener, KeyListener{
	...
	JLabel label1, label2, label3, label4, label5, label6, label7, label8, userNameLabel, passwordLabel,selectUserlb;
	Font font = new Font("Cambria", Font.BOLD , 20);
	JTextField userNameField;
	JPasswordField passwordField;
	Cursor c; 
	JCheckBox checkBox;
	JRadioButton admin, employee, hr; 
	...
	public LoginPage (){
		...
		loginButton.setBounds(160, 470, 200, 35);
		...
		// lebel
		selectUserlb = new JLabel("Select an user: ");
        selectUserlb.setBounds(20, radioYPos, 200, 50);
        selectUserlb.setFont(new Font("Arial", Font.PLAIN, 18));
		
		// admin
		admin = new JRadioButton("Admin");
		admin.setBounds(radioXPos , radioYPos, 80, 50 );
		admin.setFont(new Font("Arial", Font.PLAIN, 18));
		
		radioXPos +=radioXGap;

		// employee
		employee = new JRadioButton("Employee");
		employee.setBounds(radioXPos , radioYPos, 110, 50 );
		employee.setFont(new Font("Arial", Font.PLAIN, 18));
		
		radioXPos +=radioXGap*1.37;

		// HR
		hr = new JRadioButton("HR");
		hr.setBounds(radioXPos , radioYPos, 80, 50 );
		...
		rightPanel.add(employee);
		rightPanel.add(hr);
		rightPanel.add(selectUserlb);		
